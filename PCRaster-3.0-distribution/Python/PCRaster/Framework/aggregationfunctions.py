...
class PercentileCalculator:
  ...
  def run(self, row, col, array):
    if len(array) > 0:
      array = numpy.sort(array)
      for i in range(len(self.d_percentiles)):
        value = pcrstat.percentile(array, self.d_percentiles[i])
        self.d_fields[i].setCell(value, clone().convert(row, col))
  ...
  def run(self, arrays):
    ...
    while c < nrCells:
      if len(arrays[c]) > 0:
        arrays[c] = numpy.sort(arrays[c])

        # For each percentile.
        for p in range(len(self.d_percentiles)):
          # Calculate percentile.
          value = pcrstat.percentile(arrays[c], self.d_percentiles[p])
          # Write resulting cell value.
...
def probability(name, sampleNumbers):
  ...
  name             Name of the (boolean) raster for which each sample has a
                   realization.
...
def percentile(name, sampleNumbers, percentiles):
  """Calculates a percentile for each cell.

  name             Name of the scalar raster for which each sample has a
                   realization.
  sampleNumbers    List of numbers of samples to aggregate.
  percentiles      Percentile or list of percentiles to calculate. Percentiles
                   range from [0.0, 1.0].
  Returns a raster or a list of rasters with percentiles.
  """
  # return aggregateSPerCell(name, sampleNumbers, PercentileCalculator(percentiles))
