...
import os, signal, time, sys, math, shutil, re
import types
import shellscript, forkscript, utils, system
import atexit
...
def generateNameS(name, sample):
  """Returns a filename based on the name and sample number passed in.
...
def generateNameST(name, sample, timestep):
  """Returns a filename based on the name, sample number and time step.
...
## \brief Base class for frameworks
#
...
class FrameworkBase(shellscript.ShellScript):
  ...
  def __init__(self):
    ...
    self._d_silentModelOuput = False
    self._d_silentFrameworkOuput = True
  ...
  def _silentModelOuput(self):
    return self._d_silentModelOuput
  ...
  def _quiet(self):
    return FrameworkBase._d_quiet

  ## \brief Enable/disable all framework output to stdout
  #
  # If
  # Default is set to False
  # \param[in] quiet True/False
  def setQuiet(self, quiet):
    FrameworkBase._d_quiet = quiet

  ## \brief Trace framework output to stdout
  #
  # If tracing is enabled the user will get a detailed framework output in a XML style.
  # Default is set to False.
  ...
  def _generateName(self, name):
    """Returns a filename based on the name and the current sample number and time step.
    ...
    if self._inSample() and self._inTimeStep():
      name = self._generateNameST(name, self.currentSampleNumber(), self.currentTimeStep())
    elif self.inSample():
  ...
  def _readmapNew(self, name, style=1):
    directoryPrefix = ""
    nameSuffix = ".map"
    newName = ""
  ...
  def _assertAndThrow(self, expression, message):
