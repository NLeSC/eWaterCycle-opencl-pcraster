...
//! Install the DevelopersLicense
/*!

 The \a developersLicenseXMLString contains the contents of
 the PCRasterLicense file containing all licensed features purchased by
 you. To use the PCRaster API it should at least contain the
 DevelopersLicense feature.

 pcr_installDevelopersLicense() must always precede any other calls of the
 PCRaster API.

 <b>
 The PCRasterLicense file contents may not be distributed
 as a separate file, outside the organization of the license holder.
 The sole exception is that
 the contents of the PCRasterLicense file can be distributed
 as a string in a binary file that is not intended to be read by humans.
 Examples of these binary files are a Windows dll or Java class file.
 </b>

 The above means that the PCRasterLicense file must be compiled as a
 string constant in the program that uses the API.

 <b>Important for software testing:</b> The PCRaster API will add the features
 found in \a developersLicenseXMLString to the licensed features found in the
 PCRasterLicense file that may be present with the PCRaster distribution.
 Rename a possible PCRasterLicense file to test the setup where the user does
 not have its own PCRasterLicense.

 \param developersLicenseXMLString  C-string (Ansi 1-byte char set) with license contents.
 \returns 0 if no error, a pointer to a C-string with the error message in
            case of an error.

 \code
  // change " within PCRasterLicense file to ' if necessary
 ...
 */
 ...
 const char*
#endif
  pcr_installDevelopersLicense(const char* developersLicenseXMLString);
 ...
 *
 *  \note call pcr_installDevelopersLicense() first
 *
