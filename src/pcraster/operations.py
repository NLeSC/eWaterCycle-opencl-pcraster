...
import pcraster.opencl as opencl
...
def pcrbadd(arg1, arg2):
  try:
    ...
    if isinstance(arg1, _pcraster.Field) and isinstance(arg2, _pcraster.Field) and arg1.isSpatial() and arg1.dataType() == _pcraster.Scalar and arg2.dataType() == _pcraster.Scalar and arg2.isSpatial():
        clresult = opencl.cloperations.badd(arg1, arg2)
        ...
        diffmap = abs(pcrbmin(clresult,pcrresult))
        maxdiff = mapmaximum(diffmap)
        maxdiffValue = _pcraster.cellvalue(maxdiff, 1, 1)[0]

        if (maxdiffValue > 0.00001):
            print "max diff between pcr and cl is ", maxdiffValue

            _pcraster.report(arg1, "arg1.map")
            _pcraster.report(arg2, "arg2.map")

            _pcraster.report(clresult, "cl.map")
            _pcraster.report(pcrresult, "pcr.map")
            _pcraster.report(diffmap, "diff.map")
            print clresult
            print pcrresult
            import sys
            sys.exit(1)
