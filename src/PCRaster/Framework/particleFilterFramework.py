...
from frameworkBase import FrameworkBase, FrameworkError
...
class ParticleFilterFramework(FrameworkBase):
    # # \brief Constructor
    ...
    def _cloneData(self, samplesToClone):
    ...
    # # \brief Returns the weight of a particle
    ...
    def _setParticleWeight(self, sample, weight):
        assert self._d_particleWeights
        assert sample >= self._firstSampleNumber()
        assert sample <= self._lastSampleNumber()
