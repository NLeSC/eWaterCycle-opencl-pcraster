...
def sendMail(serverName, fromAddress, toAddress, subject, text="", \
         html="", attachments=[]):

    try:
        if html or attachments:
            message = createMultiPartMailMessage(fromAddress, toAddress, subject, \
               text, html, attachments)
        else:
            message = createMailMessage(fromAddress, toAddress, subject, text)
        ...
        raise Exception("Error while mailing message to \"%s\": %s" % \
             (toAddress, str(exception)))
...
# encoding is in iso-8859-1 format.
def createMultiPartMailMessage(fromAddress, toAddress, subject, text, \
         html, attachments):

    import base64, MimeWriter
    import mimetools
    import cStringIO

    assert html or attachments

    buffer = cStringIO.StringIO()  # output buffer for our message
    writer = MimeWriter.MimeWriter(buffer)
    ...
    if text:
        ...
        txtin = cStringIO.StringIO(text.encode("iso-8859-1"))
        part = writer.nextpart()
        part.addheader("Content-Transfer-Encoding", "quoted-printable")
        body = part.startbody("text/plain", [("charset", "iso-8859-1")])
        mimetools.encode(txtin, body, "quoted-printable")
        txtin.close()

    if html:
        # start the html part of the message
        htmlin = cStringIO.StringIO(html)
        part = writer.nextpart()
        part.addheader("Content-Transfer-Encoding", "quoted-printable")
        body = part.startbody("text/html", [("charset", "iso-8859-1")])
        mimetools.encode(htmlin, body, "quoted-printable")
        htmlin.close()

    for attachment in attachments:
        part = writer.nextpart()
        part.addheader("Content-Transfer-Encoding", "base64")
        body = part.startbody("%s; name=%s" % (attachment[0], \
             os.path.basename(attachment[1])))
        base64.encode(open(attachment[1], "rb"), body)
...
#   server = smtplib.SMTP("pop.geog.uu.nl")
#   server.sendmail('k.dejong@geog.uu.nl', 'k.dejong@geog.uu.nl', message)
#   server.quit()
